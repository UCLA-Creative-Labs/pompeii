...
Thanks for your interest in contributing to `pompeii`! ❤️
...
```
$ git clone https://github.com/uclaacm/pompeii.git
$ cd pompeii
$ yarn install
```
...
We use [eslint](.eslintrc.js) for `.js` and `.jsx` files and [stylelint](.stylelintrc.json) for `.css` and `.scss` files. 
...
We use ESLint for our TypeScript files (`**/*.js` and `**/*.jsx`). ESLint has really fantastic functionality for linting JavaScript code so naturally we picked it as our linter for our code.
...
`Project` is configured to use a pipeline to merge changes to `main`. Humans aren't perfect, so we abstracted all our merging to a pipeline managed by a bot, [Mergify](https://mergify.io/). 
